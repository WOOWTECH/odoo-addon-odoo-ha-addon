...
1. Copy the contents of `src/` into your Odoo addons directory, naming it `odoo_ha_addon`:
   ```bash
   cp -r src/ /path/to/odoo/addons/odoo_ha_addon/
   ```

2. Update the apps list in Odoo:
   - Go to **Apps** and click **Update Apps List**

3. Search for and install **WOOW Dashboard**

4. The Python dependency `websockets` is automatically installed via the `pre_init_hook` -- no manual pip install is required.
...
```
woow-odoo-ha-addon/
├── src/                      # Odoo addon source (odoo_ha_addon)
...
1. 將 `src/` 目錄內容複製到 Odoo 的 addons 路徑，命名為 `odoo_ha_addon`：
   ```bash
   cp -r src/ /path/to/odoo/addons/odoo_ha_addon/
   ```
...
3. 搜尋並安裝 **WOOW Dashboard**

4. Python 相依套件 `websockets` 會透過 `pre_init_hook` 自動安裝，無需手動執行 pip install。
