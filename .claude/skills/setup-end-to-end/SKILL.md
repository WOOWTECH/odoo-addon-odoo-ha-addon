...
git worktree add ../odoo_ha_addon.worktrees/<feature-name> -b epic/<feature-name>

# 範例：
# git worktree add ../odoo_ha_addon.worktrees/workspace-api -b epic/workspace-api
```
...
```bash
cd ../odoo_ha_addon.worktrees/<feature-name>
```
...
✅ Worktree 環境配置完成
  ...
  Project:      odoo_ha_addon_epic_workspace_api
  Port:         8234
...
3. 啟動 Docker Compose
4. 等待 Odoo 服務就緒
5. **自動建立資料庫並安裝 `base` + `odoo_ha_addon`**（首次啟動）
6. 顯示訪問 URL
...
```bash
# 更新模組
docker compose exec web odoo -d <DB_NAME> -u odoo_ha_addon --stop-after-init
```
...
docker compose exec web odoo \
  --test-enable \
  --test-tags /odoo_ha_addon/models \
  --stop-after-init \
  -d woow_epic_workspace_api
```
...
cd ../odoo_ha_addon.worktrees/feature-a
./scripts/start-dev.sh
# → http://localhost:8234

# Terminal 2 - Feature B
cd ../odoo_ha_addon.worktrees/feature-b
./scripts/start-dev.sh
# → http://localhost:8501
```
...
# 刪除 worktree
git worktree remove ../odoo_ha_addon.worktrees/<feature-name>
...
```bash
# 確認 addon 路徑掛載
docker compose exec web ls -la /mnt/extra-addons/odoo_ha_addon
...
cd /mnt/extra-addons/odoo_ha_addon
```
...
```bash
# 1. 建立 worktree
cd ~/Documents/woow/AREA-odoo/woow-addons/odoo_ha_addon
git worktree add ../odoo_ha_addon.worktrees/workspace-api -b epic/workspace-api

# 2. 切換到 worktree 並啟動（自動設定環境 + 建立資料庫）
cd ../odoo_ha_addon.worktrees/workspace-api
./scripts/start-dev.sh
# 首次啟動會自動：
# - 生成 .env 和 odoo.conf
# - 建立資料庫並安裝 odoo_ha_addon
...
# 5. 如有結構性變更，更新模組
docker compose exec web odoo -d woow_epic_workspace_api -u odoo_ha_addon --stop-after-init
